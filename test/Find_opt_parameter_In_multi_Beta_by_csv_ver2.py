...
matched_genes_folder = "matched_genes"
os.makedirs(detail_output_folder, exist_ok=True)
os.makedirs(matched_genes_folder, exist_ok=True)
...
def run_simulation(G, EPSILON, DELTA, N_BETA, save_detail=False, detail_file=None):
    ...
    return [node for node, _ in top_100_nodes]  # ✅ Đổi thành list để lấy giao cắt cụ thể

# ✅ MAIN

def main():
    # ✅ Chuẩn hóa tên gene đúng khi đọc
    truth_genes = set(
        pd.read_csv(truth_file)["Gene"].astype(str).str.strip().str.upper()
    )
    ...
    for _, row in param_df.iterrows():
        ...
        for file in files:
            ...
            # ✅ Nếu file chi tiết đã tồn tại → bỏ qua
            if os.path.exists(detail_path):
                print(f"  ⚠️ Đã tồn tại {detail_filename}, bỏ qua.")
                continue

            top_genes = run_simulation(
                G, EPSILON, DELTA, N_BETA,
                save_detail=True,
                detail_file=detail_path
            )

            # ✅ Chuẩn hóa gene trước khi so khớp
            top_genes_std = set(g.strip().upper() for g in top_genes)
            match = len(truth_genes & top_genes_std)
    ...
    if not df.empty:
        df = df.sort_values("Matching_Rate", ascending=False)
        output_file = "../grid_search_result.csv"
        if os.path.exists(output_file):
            df.to_csv(output_file, mode="a", header=False, index=False)
        else:
        ...
        print("⚠️ Không có tổ hợp nào được chạy mới, không ghi file.")
        print("✅ Đã lưu kết quả vào grid_search_result.csv")
